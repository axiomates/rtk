...
pub(crate) mod tests {
    ...
    fn stdout_cmd(text: &str) -> Command {
        if cfg!(windows) {
            let mut cmd = Command::new("cmd");
    ...
    fn stderr_cmd(text: &str) -> Command {
        if cfg!(windows) {
            let mut cmd = Command::new("cmd");
            cmd.args(["/C", &format!("echo {} 1>&2", text)]);
            cmd
        } else {
            let mut cmd = Command::new("sh");
    ...
    fn stdout_stderr_cmd(stdout: &str, stderr: &str) -> Command {
        if cfg!(windows) {
            let mut cmd = Command::new("cmd");
            cmd.args(["/C", &format!("echo {} & echo {} 1>&2", stdout, stderr)]);
            cmd
        } else {
            let mut cmd = Command::new("sh");
            cmd.args(["-c", &format!("echo {}; echo {} >&2", stdout, stderr)]);
            cmd
        }
    }

    fn raw_cap_stdout_cmd() -> Command {
        if cfg!(windows) {
            let mut cmd = Command::new("powershell");
            cmd.args([
                "-NoProfile",
                "-Command",
                "$chunk = 'a' * 80; for ($i = 0; $i -lt 140000; $i++) { [Console]::Out.WriteLine($chunk) }",
            ]);
            cmd
        } else {
            let mut cmd = Command::new("sh");
    ...
    fn raw_cap_stderr_cmd() -> Command {
        if cfg!(windows) {
            let mut cmd = Command::new("powershell");
            cmd.args([
                "-NoProfile",
                "-Command",
                "$chunk = 'a' * 80; for ($i = 0; $i -lt 140000; $i++) { [Console]::Error.WriteLine($chunk) }",
            ]);
            cmd
        } else {
            let mut cmd = Command::new("sh");
    ...
    fn test_exit_code_zero() {
        let status = success_cmd().status().unwrap();
        assert_eq!(status_to_exit_code(status), 0);
    }

    #[test]
    fn test_exit_code_nonzero() {
        let status = failure_cmd().status().unwrap();
    ...
    fn test_run_streaming_raw_cap_at_10mb() {
        let mut cmd = raw_cap_stdout_cmd();
    ...
    fn test_run_streaming_stderr_cap_at_10mb() {
        let mut cmd = raw_cap_stderr_cmd();
        let result = run_streaming(&mut cmd, StdinMode::Null, FilterMode::CaptureOnly).unwrap();
    ...
    fn test_run_streaming_raw_contains_stdout() {
        let mut cmd = stdout_cmd("test_output_xyz");
        let result = run_streaming(&mut cmd, StdinMode::Null, FilterMode::CaptureOnly).unwrap();
    ...
    fn test_run_streaming_capture_only_filtered_equals_raw() {
        let mut cmd = stdout_cmd("check_equality");
        let result = run_streaming(&mut cmd, StdinMode::Null, FilterMode::CaptureOnly).unwrap();
    ...
    fn test_exec_capture_success() {
        let mut cmd = stdout_cmd("hello_capture");
    ...
    fn test_exec_capture_combined() {
        let mut cmd = stdout_stderr_cmd("out_msg", "err_msg");
        let result = exec_capture(&mut cmd).unwrap();
