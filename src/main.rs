...
/// Global kubectl flags forwarded to all subcommands (--context, --kubeconfig, -n, -l, --as)
...
impl KubectlGlobalArgs {
    /// Convert the global flags into kubectl CLI arguments, in canonical order.
    fn to_args(&self) -> Vec<String> {
        ...
        if let Some(imp) = &self.impersonate {
...
mod tests {
    ...
    fn test_kubectl_global_args_to_args() {
        ...
        assert_eq!(
            args,
            vec!["--context", "ctx-1", "-n", "prod", "--as", "admin"]
        );
    }

    #[test]
    fn test_kubectl_pods_all_namespaces_with_context() {
        let cli =
            Cli::try_parse_from(["rtk", "kubectl", "pods", "--context", "prod", "-A"]).unwrap();
        match cli.command {
            Commands::Kubectl {
                command: KubectlCommands::Pods { global, all },
            } => {
